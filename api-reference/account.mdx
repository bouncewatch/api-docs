...
| `POST /api/v1/account/webhook` | Configure webhooks |
...
<CodeGroup>
```bash cURL
curl -X GET "https://api.bouncewatch.com/api/v1/account/usage?period=month" \
  -H "X-API-Key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(
  'https://api.bouncewatch.com/api/v1/account/usage?period=month',
  { headers: { 'X-API-Key': 'YOUR_API_KEY' } }
);
```
...
```php PHP
$ch = curl_init('https://api.bouncewatch.com/api/v1/account/usage?period=month');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, ['X-API-Key: YOUR_API_KEY']);
$response = curl_exec($ch);
$data = json_decode($response, true);
