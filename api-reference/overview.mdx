...
async function getCompanyData(domain) {
  try {
    const response = await fetch(
      `https://api.bouncewatch.com/v1/company/${domain}`,
...
function getCompanyData($domain) {
    $ch = curl_init("https://api.bouncewatch.com/v1/company/{$domain}");
...
For endpoints that return lists (like request history), pagination is supported:
...
```bash
GET /v1/account/history?page=2&limit=25
```
...
The API version is included in the URL path. Currently, we're on **v1**.
...
<Note>
  We maintain backward compatibility within major versions. Breaking changes will be announced in advance and released under a new version.
</Note>
